#ifndef REUSABILITY_FUNCTIONS_TAPS_H
#define REUSABILITY_FUNCTIONS_TAPS_H

/*
This header just contains the functions which will be used in p2p_scenarioReusable.cc
Adapted to use taps in both sides
*/
...
void ManageMiddleLinks(json data, PointToPointHelper pointToPoint, vector<NetDeviceContainer> &Devs, NodeContainer &nodes, int &auxnodes)
{
  ...
  {
    auxnodes = 3 + (int)data.at("NumLinks");
    nodes.Create(auxnodes);
    // NS_LOG_UNCOND ("Nodes Created");
...
void BackgroundTrafficSetup(ApplicationContainer &app, ApplicationContainer &sinkApps, json data,
                            vector<Ipv4InterfaceContainer> &ifacesP2p, NodeContainer nodes)
{
  ...
  for (int i = 0; i < (int)data.at("NumLinks"); i++)
  {
    if (data.at("Links")[i]["Background"])
    {
      ...
      sinkApps.Add(packetSinkHelper.Install(nodes.Get(i + 2))); // Node 3: Destination node
      ...
      app.Add(Sender.Install(NodeContainer{nodes.Get(i + 1)}));
...
void SettingUpModelLinks(json data, const vector<NetDeviceContainer> &Devs, float timevalue)
{
  int aux1 = 0, aux2 = 0;
