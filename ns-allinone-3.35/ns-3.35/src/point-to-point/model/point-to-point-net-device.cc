...
#include <iostream>
#include <string>
#include "ns3/string.h" // para tipo stringvalue
...
namespace ns3
{
  ...
  PointToPointNetDevice::GetTypeId(void)
  {
    static TypeId tid = TypeId("ns3::PointToPointNetDevice")
                            ...
                            .AddTraceSource("MacPromiscRx",
                                            ...
                                            "ns3::Packet::TracedCallback")
                            .AddTraceSource("Inqueue", // añadido
                                            "Trace source indicating a packet has ben enqueued "
                                            " by this device",
                                            MakeTraceSourceAccessor(&PointToPointNetDevice::m_queuedTrace),
                                            "ns3::Packet::TracedCallback")
                            .AddTraceSource("Outqueue", // añadido
                                            "Trace source indicating a packet has ben dequeued "
                                            " by this device",
                                            MakeTraceSourceAccessor(&PointToPointNetDevice::m_dequeuedTrace),
                                            "ns3::Packet::TracedCallback")
  ...
  PointToPointNetDevice::AddHeader(Ptr<Packet> p, uint16_t protocolNumber)
  {
    ...
    // std::cout<< "antes de AddHeader en NetDevice, p:size= " <<p->GetSize() << std::endl;
    p->AddHeader(ppp);
    // std::cout << "despues de AddHeader en NetDevice, p_size= " <<p->GetSize() << std::endl;
  }
  ...
  {
    NS_LOG_FUNCTION(this);
    NS_LOG_DEBUG(this << "| " << Simulator::Now().GetSeconds() << " | Seting data rate to " << bps << "antes era " << m_bps);

    if (m_bps == 0 and bps > 0)
    {
      ...
      if (m_txMachineState == READY)
      {
        ...
        {
          m_dequeuedTrace(p); //    **********Traza outcola********
        }
        NS_LOG_DEBUG("Donde el fallo: ms:  " << Simulator::Now().GetMilliSeconds() << " pkt uid, size: " << p->GetUid() << "\t" << p->GetSize());
  ...
  PointToPointNetDevice::TransmitComplete(void)
  {
    ...
    if (m_bps == 0)
    {
      NS_LOG_LOGIC("Data rate is 0, 'm not checking the queue");
      return;
    }
      ...
      uint16_t protocolNumber)
  {
    ...
    // std::cout << "**********before enqueue, in send function time : "<< Simulator::Now().GetSeconds()<<  std::endl;
    //
    ...
    //
    // std::cout << "hello, tenemos actual en paquetes: " << m_queue->GetCurrentSize()<< " y maximo: " << m_queue->GetMaxSize() << std::endl;

    if (m_queue->Enqueue(packet))
    {
      // std::cout << "tras entrar en cola tenemos: " << m_queue->GetCurrentSize()<< " y maximo: " << m_queue->GetMaxSize() << std::endl;
      m_queuedTrace(packet); // packet ha entrado en cola ***                aqui trace ********
      //
      // If the channel is ready for transition we send the packet right now
      //
      if (m_txMachineState == READY and m_bps > 0)
      {
        packet = m_queue->Dequeue();
        m_dequeuedTrace(packet); //                     ****Traza fin cola*********
        m_snifferTrace(packet);
        m_promiscSnifferTrace(packet);
    ...
    // std::cout << "paquete no entro en cola" << std::endl;
