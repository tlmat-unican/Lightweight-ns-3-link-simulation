...
Sim_name = "{}{}_SyntheticLMS".format(OutcomeFolder, Transport_model)

if ConfigPath != "":
    ConfigPathStr = "--json-path={}".format(ConfigPath)

if Sim_name != "":
    Config_Out = "--out-folder-path={}".format(
        "../../scripts/" + Sim_name + "/")
...
def RunNs3(ns3Path, scenario, ConfigPath, folder_name, Config_model_value):
    ...
    if ConfigPath != "":
        ConfigPathStr = " --json-path=.{}".format(ConfigPath)
        cmd = 'cd {} && sudo ./waf --run "{}{} {} {}"'.format(
            ns3Path, scenario, ConfigPathStr, folder_name, Config_model_value)
    else:
        cmd = 'cd {} && sudo ./waf --run "{}{}"'.format(
            ns3Path, scenario, folder_name)
    print(cmd + "\n")
    os.system(cmd)
